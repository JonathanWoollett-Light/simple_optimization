...
use itertools::{izip, FoldWhile::{Continue, Done}};
...
impl CoolingSchedule {
    ...
    fn iterations(&self, t_start: f64, t_min: f64) -> u32 {
        match self {
...
>(
    ...
    // Gets ranges in f64
    let f64_ranges: Vec<Range<f64>> = ranges.iter().map(|r| r.start.to_f64().unwrap()..r.end.to_f64().unwrap()).collect();
    ...
    let iterations = cooling_schedule.iterations(starting_temperature, minimum_temperature);
    ...
    fn sample_normal<R: Rng + ?Sized, T: num::FromPrimitive>(range: &Range<f64>, distribution: &Normal<f64>, rng: &mut R) -> T {
